...
func GetTransactions(filters map[string]string) (txnModel.PaginatedResponse, error) {
	...
	if filters["start_date"] != "" {
		...
		database.GetDB().Model(&txnModel.Transactions{}).Where(filters).Count(&totalRecords)
...
func SaveOrUpdateTransaction(transaction *txnModel.Transactions) (*txnModel.Transactions, error) {
